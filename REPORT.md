...
**2.8 Refusal is code first, weights second.** Seven prohibitions (pesticide mixing rates, pre-harvest intervals, veterinary drugs, human medicines
...
## 7. The console in use

The offline console (`app/`, §2.6) demonstrating the build. All screenshots are
of the live system answering on this machine with networking unavailable; the
landing view's background video is embedded inside the single served HTML file,
so even it renders offline.

![The console landing view: full-bleed dune footage, live status, and count-up statistics, all served from one offline file](docs/screenshots/01-console-hero.jpg)

![A cited answer: the retrieval pipeline stepper, coverage 0.86 against the 0.72 refuse line, live tokens-per-second meter, and per-passage retrieval-strength bars](docs/screenshots/02-cited-answer.jpg)

![An evidence passage opened: the exact manual excerpt behind citation 1, on-device](docs/screenshots/03-evidence-passage.jpg)

![A hazard-class refusal: the coverage gauge stops short of the refuse line and the turn is stamped REFUSED IN CODE; the refusal text names who can actually help](docs/screenshots/04-safety-refusal.jpg)

![The working console in the dark theme: the shelf drawn as 56 spines sized by their real indexed passage counts](docs/screenshots/05-console-dark.jpg)
